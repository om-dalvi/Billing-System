...
m:
	...
	case 1:
		...
		if (email = "omdalvi@gmail.com" && password = "billadmin")
		{
			administrator();
		}
		else
...
void shopping::rem()
{
	...
	else
	{
		...
		data1.close();
		remove("database.txt");
		raname("database1.txt", "database.txt");
	...
	if (!data)
	{
		...
		do
		{
			...
			for(int i = 0; i < c; i++)
			{
				if (arr[c] == arrc[i])
				{
					cout << "\n\n Duplicate prodct code. Please try again!";
					goto m;
				}
			}
			c++;
		...
		cout << "\n Product name \t Product quantity\ tPrice\ tAmpunt \t tAmount with disount \n";
